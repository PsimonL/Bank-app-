...
class EmployeePanel():
    ...
    def employee_approved(self, type, application):
        ...
        if type == "account":
            ...
            prev_value=mySQL_connection("select value from wallets where accountNumber="+id)[0]
            loan_value=mySQL_connection("select value from loans where accountNumber="+id)[0]
            value=str(prev_value+loan_value)
            mySQL_connection("UPDATE loans SET confirmed = 1, interest=5 where accountNumber="+id,True)
            mySQL_connection("UPDATE wallets SET value = "+value+" where accountNumber="+id,True)
            self.reinitialize()
        elif type == "card":
