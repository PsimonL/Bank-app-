    ...
    def sign_in_client(self):
        ...
        def sign():
            ...
            if user[2] == data['password']and user[7]==1:
                ...
                print("Wait for a employee approval")
            else:
                self.login_counter += 1
                print("Wrong login or password!")

        get_logged = self.my_button(login_container, txt="Log in", command=sign)
        recover_password_button = self.my_button(login_container, txt="Recover password", command=recover_password)
        recover_password_button.grid(row=4, column=1)
